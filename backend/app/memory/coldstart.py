...
async def run_coldstart(user_id: int, provider, store) -> dict:
    ...
    try:
        reply = await create_llm().ainvoke([SystemMessage(content=prompt), HumanMessage(content=json.dumps(sources, ensure_ascii=False)[:24000])])
        ...
        persona, partial = dict(DEFAULT_PERSONA), True
