...
脚本会构建前端、创建 FastAPI/RPGJS 的 systemd 服务、生成 Nginx 配置并尝试申请 HTTPS。它默认使用 `duanzhiwojing.site`、`game.duanzhiwojing.site` 和服务器公网 IP `116.30.139.215`，也会交互询问这些值及 OAuth 凭证。OAuth 凭证只写入服务器 `/etc/zhihu.env`（权限 600），不会写入仓库；运行前请先将两个域名的 DNS A 记录指向服务器公网 IP。当前版本的 OAuth 路由仍是安全占位接口，真实授权流程需在后续版本启用。
