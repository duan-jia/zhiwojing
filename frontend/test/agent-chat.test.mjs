...
test('avatar sprites open a persistent, failure-safe Agent chat overlay', async () => {
  ...
  assert.match(html, /id="autonomy-mode"/)
  ...
  assert.match(clientConfig, /setupAutonomyStatus/)
  assert.doesNotMatch(clientConfig, /applyAutonomyPositionSync/)
  assert.match(server, /id: 'avatar-su-wan'/)
  assert.match(server, /id: 'avatar-zhou-bo'/)
  assert.match(server, /sessionExpiryTime = 0/)
  assert.match(styles, /\.chat-panel/)
  assert.match(styles, /\.autonomy-mode/)
