...
test('login shell gates RPGJS startup and preserves guest and OAuth paths', async () => {
  const [html, standalone, login, styles] = await Promise.all([
    readFile(join(projectRoot, 'index.html'), 'utf8'),
    readFile(join(projectRoot, 'src', 'standalone.ts'), 'utf8'),
  ...
  assert(standalone.indexOf('await showLogin()') < standalone.indexOf('startGame('))
  assert.match(login, /游客身份进入/)
