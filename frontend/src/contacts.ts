...
export function setupContacts(userId: number) {
  ...
  const render = async () => {
    ...
    root.innerHTML = `<section class="contacts-panel" role="dialog" aria-modal="true" aria-labelledby="contacts-title">
      ...
      <div class="contacts-layout"><nav aria-label="联系人">${contacts.length ? contacts.map(item => `<button class="contact-row${active?.id === item.id ? ' active' : ''}" data-id="${item.id}"><i class="${item.online ? 'online' : ''}"></i><span><strong>${escapeHtml(item.name)}</strong><small>${escapeHtml(item.lastMessage || '还没有消息')}</small></span>${item.unread ? `<b>${item.unread}</b>` : ''}</button>`).join('') : '<p class="contacts-empty">完成一次对话后，对方会出现在这里。</p>'}</nav>
      <main>${active ? `<div class="thread-title"><span><strong>${escapeHtml(active.name)}</strong><small>${active.online ? (active.humanControlled ? '在线 · 本人操控' : '在线 · 分身代理') : '离线'}</small></span><button class="contact-status">删除</button></div><div class="thread-messages">${messages.map(item => `<article class="${item.senderId === userId ? 'mine' : ''}"><small>${item.senderKind === 'agent' ? '分身' : item.senderId === userId ? '你' : escapeHtml(active!.name)}</small><p>${escapeHtml(item.content)}</p></article>`).join('') || '<p class="contacts-empty">发一条消息开始远程对话。</p>'}</div>${notice ? `<p class="thread-notice">${escapeHtml(notice)}</p>` : ''}<form class="thread-form"><input maxlength="4000" required placeholder="发送远程消息…"><button>发送</button></form>` : '<p class="contacts-empty thread-placeholder">选择一位联系人查看会话</p>'}</main></div></section>`
    ...
    root.querySelector('.contact-status')?.addEventListener('click', async () => { if (!active) return; await request(`/api/contacts/${active.id}?user_id=${userId}`, { method: 'DELETE' }); active = null; await refresh(); void render() })
  ...
  return { destroy: () => { window.clearInterval(timer); button.remove() } }
}
