...
客户端通过 `addSpriteComponentInFront` 把 CanvasEngine 血条直接挂到每个远端玩家精灵；它不依赖屏幕坐标投影，随角色和镜头移动。血条显示实时 `当前 HP/最大 HP`，归零时显示「倒地」并灰化。组件排除当前玩家和所有事件，因此地图 NPC/地标既没有血条，也不会成为战斗目标。自己的左下角 HP HUD 保持原布局；窄屏沿用固定左下角位置，不覆盖右侧通讯录/对话区域。
